...
{
  gzFile memory_trace_file;
  ostringstream oss;
  oss << bench << "_memaddr.gz";
  memory_trace_file = gzopen(oss.str().c_str(), "w");
  for(unsigned int i = 0; i < mem_address_vector.size(); ++i)
    gzprintf(memory_trace_file, "%d,%d\n",  mem_address_vector.at(i), mem_address_times.at(i));
  gzclose(memory_trace_file);
}
...
void traceProfiler::output_varid( string bench, int which_varid)
{
  ...
  gzFile varid_file;
  varid_file = gzopen(oss.str().c_str(), "w");
  for(unsigned i = 0; i < varid_vector.size(); ++i)
    gzprintf(varid_file, "%d\n",  varid_vector.at(i)[which_varid-1]);
  gzclose(varid_file);
}
...
void traceProfiler::output_parvalue( string bench, int which_parid)
{
  ...
  gzFile parvalue_file;
  parvalue_file = gzopen(oss.str().c_str(), "w");
  for(unsigned i = 0; i < parvalue_vector.size(); ++i)
    gzprintf(parvalue_file, "%s\n", parvalue_vector.at(i)[which_parid-1].c_str());
  gzclose(parvalue_file);
}
...
void traceProfiler::output_partype( string bench, int which_parid)
{
  ...
  gzFile partype_file;
  partype_file = gzopen(oss.str().c_str(), "w");
  for(unsigned i = 0; i < partype_vector.size(); ++i)
    gzprintf(partype_file ,"%d\n",  partype_vector.at(i)[which_parid-1]);
  gzclose(partype_file);
}
