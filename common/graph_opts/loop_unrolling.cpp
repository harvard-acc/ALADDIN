...
void LoopUnrolling::optimize() {
  ...
  for (auto node_it = exec_nodes.begin(); node_it != exec_nodes.end();
    ...
    if (!next_node)
      continue;
    if (node->is_branch_op() && next_node->get_basic_block()->is_preheader()) {
      // The debug information generated by LLVM-6.0 can be inaccurate at
      // setting the line number of a preheader, which is crucial for Aladdin
      // to apply loop unrolling. As a heuristic, we will use the intermediate
      // branch before the preheader block as a loop boundary. In this case, to
      // get the target loop depth, we need to find the node after the preheader
      // block instead of the next node.
      unsigned next_node_id = next_node->get_node_id();
      while ((next_node = program.getNextNode(next_node_id))) {
        if (!next_node->get_basic_block()->is_preheader())
          break;
      }
    }
