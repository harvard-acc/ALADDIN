...
class NodeVisitor : public boost::default_bfs_visitor {
 public:
  ...
  void discover_vertex(const Vertex& v, const Graph& g) const {
    ...
    out_edge_iter edge_it, end;
    for (boost::tie(edge_it, end) = out_edges(v, g); edge_it != end; ++edge_it) {
      Vertex target_vertex_orig = target(*edge_it, g);
      ...
      if (existing_nodes->find(target_id) == existing_nodes->end()) {
        ...
        unsigned edge_weight = get(boost::edge_name, g, *edge_it);
        add_edge(
            new_vertex, target_vertex, EdgeProperty(1), *new_graph);
        existing_nodes->operator[](target_id) = target_vertex;
        (*num_nodes_visited)++;
      }
    }
  }
  void initialize_vertex(const Vertex& v, const Graph& g) const {}
  void examine_vertex(const Vertex& v, const Graph& g) const {}
  void examine_edge(const Edge& e, const Graph& g) const {}
  void finish_vertex(const Vertex& v, const Graph& g) const {}
