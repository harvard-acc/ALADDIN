...
def main(directory, kernel, algorithm, part, unroll, pipe):
  ...
  if bench == 'aes-aes':  # TODO need to fix
    ...
    config.write('unrolling,aes256_encrypt_ecb,198,%s\n' %(unroll)) #trip_cnt = 32
    config.write('unrolling,aes256_encrypt_ecb,201,%s\n' %(unroll)) #trip_cnt = 8
    config.write('unrolling,aes256_encrypt_ecb,207,%s\n' %(unroll)) #tc = 13
    ...
    if unroll >= 10:
      config.write('flatten,bfs,50\n')
    else:
      config.write('unrolling,bfs,68,%s\n' %(unroll))               # trip_cnt = 10
      #config.write('unrolling,bfs,50,%s\n' %(unroll))               # trip_cnt = 10
    config.write('flatten,bfs,53\n')
    config.write('flatten,bfs,57\n')
    ...
    if unroll >= 10:                                              # trip_cnt = 10
      config.write('flatten,fft,8\n')
    else:
      config.write('unrolling,fft,8,%s\n' %(unroll))
    config.write('flatten,fft,9\n')
  elif bench == 'fft-transpose':
    if unroll >= 8:
      config.write('flatten,twiddles8,55\n')
    else:
      config.write('unrolling,twiddles8,55,%s\n' %(unroll))       # trip_cnt = 8
    ...
    if unroll >= 8:
      config.write('flatten,bbgemm,43\n')
    else:
      config.write('unrolling,bbgemm,43,%s\n' %(unroll))        # trip_cnt = 8
    config.write('flatten,bbgemm,44\n')
    config.write('flatten,bbgemm,45\n')
    config.write('flatten,bbgemm,46\n')
    config.write('flatten,bbgemm,50\n')
    ...
    if unroll >= 16:
        config.write('flatten,ss_sort,110\n')
    else:
        config.write('unrolling,ss_sort,110,%s\n' %(unroll))    # trip_cnt = 16
  elif bench == 'spmv-crs':
    config.write('unrolling,spmv,41,%s\n' %(unroll))            # trip_cnt = 494
    config.write('flatten,spmv,45\n')
  elif bench == 'spmv-ellpack':
    config.write('unrolling,ellpack,41,%s\n' %(unroll))         # trip_cnt = 494
    config.write('flatten,ellpack,43\n')
  elif bench == 'stencil-stencil2d':
    config.write('unrolling,stencil,38,%s\n' %(unroll))         # trip_cnt = 126
    config.write('flatten,stencil,39\n')
    config.write('flatten,stencil,40\n')
    config.write('flatten,stencil,41\n')
  elif bench == 'stencil-stencil3d':
    if unroll >= 30:
      config.write('flatten,stencil3d,42\n')
    else:
      config.write('unrolling,stencil3d,42,%s\n' %(unroll))     # trip_cnt = 30
    config.write('flatten,stencil3d,43\n')
    config.write('flatten,stencil3d,44\n')
  elif bench == 'viterbi-viterbi':
    config.write('unrolling,viterbi,41,%s\n' %(unroll))         # trip_cnt = 128
    config.write('flatten,viterbi,43\n')
    config.write('flatten,viterbi,44\n')
    if unroll >= 32:
      config.write('flatten,viterbi,55\n')
    else:
      config.write('unrolling,viterbi,55,%s\n' %(unroll))         # trip_cnt = 32
  else:
...
if __name__ == '__main__':
  ...
  unroll = sys.argv[5]
  pipe = sys.argv[6]
  main(directory, kernel, algorithm, part, unroll, pipe)
